...
async function testAIAssistantTracking() {
    ...
    try {
        ...
        const { data: sessionDataChain, error: sessionError } = await supabase
            .from('ai_assistant_sessions')
            .select('*')
            .eq('session_id', testSessionId);

        const { data: jobDraftsDataChain, error: jobDraftsError } = await supabase
            .from('job_drafts')
            .select('*')
            .eq('session_id', testSessionId);

        const { data: analyticsDataChain, error: analyticsError } = await supabase
            .from('analytics')
            .select('*')
            .eq('session_id', testSessionId);

        if (sessionError || jobDraftsError || analyticsError) {
            console.log('❌ Error testing tracking chain:', sessionError?.message || jobDraftsError?.message || analyticsError?.message);
            return;
        }
