...
export async function handleQuestionsRoute(req: Request, path: string): Promise<Response> {
  ...
  try {
    ...
    if (method === 'GET' && path === '/api/questions/ai-personas') {
      return await questionController.getAIPersonas(req, path)
    }

    // POST /api/questions/ai-engagement - Generate proactive AI engagement
    if (method === 'POST' && path === '/api/questions/ai-engagement') {
      return await questionController.generateProactiveEngagement(req, path)
    }
